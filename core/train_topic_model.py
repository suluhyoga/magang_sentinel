...
def main():
    print("🧹 Analisis Dataset Topic...")
    df = pd.read_csv(DATA_PATH, sep=';').dropna()
    
    df['text'] = df['text'].astype(str).apply(preprocess_text)
    df = df[df['text'].str.len() > 10]
    df = df.drop_duplicates(subset=['text'])
    df["label"] = df["topic"].str.lower().map(LABEL_MAP)
    ...
    model = AutoModelForSequenceClassification.from_pretrained(BASE_MODEL, num_labels=5, hidden_dropout_prob=0.3)
    ...
    args = TrainingArguments(
        output_dir="./tmp_topic", eval_strategy="epoch", save_strategy="epoch",
        load_best_model_at_end=True, metric_for_best_model="f1",
        per_device_train_batch_size=16, num_train_epochs=25, # Epoch agak banyak untuk model yg dibekukan
    ...
    print("✅ Topic Model Bebas Overfitting Berhasil Disimpan!")
