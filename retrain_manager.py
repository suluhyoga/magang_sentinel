...
from config.settings import DB_PATH, MODEL_DIR_TOPIC, MODEL_DIR_SENTIMENT, MODEL_DIR_EMERGENCY
from core.ai_engine import ai_engine

BACKUP_DIR = "model/backups"

# Integrasi dengan settings.py
MODEL_CONFIG = {
    "topic": {
        "db_col": "verified_topic",
        "csv_col": "topic",
        "script": "core/train_topic_model.py",
        "model_dir": MODEL_DIR_TOPIC,
...
def retrain_model(model_type):
    ...
    try:
        ...
        print(f"⚠️ Model lama gagal dievaluasi: {e}")
        old_f1 = 0.0
    ...
    try:
        ...
        if new_f1 >= old_f1:
            ...
            return {"status": "success", "message": "Retrain berhasil", "old_score": round(old_f1, 4), "new_score": round(new_f1, 4)}
        else:
            print("❌ Model Baru Lebih Buruk. Melakukan Rollback...")
            shutil.rmtree(config["model_dir"])
            shutil.copytree(backup_path, config["model_dir"])
            ai_engine.reload_models()
            return {"status": "rollback", "message": "Retrain dibatalkan", "old_score": round(old_f1, 4), "new_score": round(new_f1, 4)}
